...
def swindow():
	window = ctk.CTkToplevel(root, fg_color="#3B8ED0")
	...
	entry = ctk.CTkEntry(window, width=350, height=35, \
		placeholder_text="Enter the name of template here", font=("Roboto", 15))
	button = ctk.CTkButton(window, text="Save and exit", fg_color="white", \
		width=200, height=30, text_color="#3B8ED0", \
		command=lambda:[save_template(entry.get()), root.destroy()])
